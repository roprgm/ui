...
- `theme.css` holds what components need (tokens, utilities, base rules for controls); `base.css` holds optional page defaults. `bun run build` writes their `registry.json` items (`theme`, `base`) from them with `scripts/registry.ts`; don't edit those items by hand.
