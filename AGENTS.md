...
- `theme.css` holds everything the components need: tokens, utilities, and base rules for the page and controls. `bun run build` writes its `registry.json` item (`theme`) with `scripts/registry.ts`; don't edit that item by hand.
