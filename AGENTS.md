...
- `theme.css` holds the values (tokens and the page) and imports `base.css`, the utilities, variants, and control rules that read them. The look goes in the theme; what every theme shares goes in the base. `bun run build` writes both into the `theme` item of `registry.json` with `scripts/registry.ts`; don't edit that item by hand.
