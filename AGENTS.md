...
- A theme in `src/themes/` imports `base.css` and draws the primitives' edges its own way: `default.css` with shadows, `lines.css` with 1px lines that take no room, `flat.css` with nothing, and `light.css` with shadows on its own light colors. An edge should take no room either: a real `border` grows any control whose height comes from its content, so draw lines as `inset 0 0 0 1px` box-shadows. It changes a fill by setting its color token again, never by redeclaring a property `base.css` sets: Tailwind orders the merged definitions of a utility by how many properties each has, so the base's could win. `bun run build` writes `default.css` and `base.css` into the `theme` item of `registry.json` with `scripts/registry.ts`; don't edit that item by hand.
...
- For one line of text that may not fit, use the `scroll-text` utility rather than `truncate`.
- Each component keeps its styles in its own class strings. `base.css` holds only primitives and utilities that aren't one component, such as `focus-ring`, `dim-disabled`, `range-thumb` (a range input's thumb, for Slider and VerticalSlider), `scroll-fade`, and `shimmer`. Rows in a popup's list take `popupItem` from `popup.tsx`.
- A theme changes tokens and primitives for what is general. For a part of one component, it targets the part's `data-slot` in a rule outside any layer, which wins over the component's utilities: `[data-slot="slider-track"] { … }`. Add `data-slot` to a part only when a theme needs it, and treat its name as public. Such a rule also wins over a caller's `className`, which then needs `!`. A theme never changes markup.
